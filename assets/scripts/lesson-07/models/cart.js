...
class Cart {
    static addProduct(id, product_price) {
        fs.readFile(cart_file_path, (err, fileContent) => {
            ...
            const existing_product_index = cart.products.findIndex(prod => parseInt(prod.id) === parseInt(id));
    ...
    static deleteProduct(id, product_price) {
        fs.readFile(cart_file_path, (err, fileContent) => {
            if (err) {
                ...
                const existing_product_index = updated_cart.products.findIndex(prod => parseInt(prod.id) === parseInt(id));
